...
CONSTANTS N, \* set of all nodes 
          ...
          PendingConsistentRead, PendingCommitResult, Committed, Aborted, PendingAbort,
          Up, Down,
...
(*
Notes about Raft
While the system is based on Raft, we do not model Raft directly in order to reduce the state space.
We do not use a "term" as all reads and writes go to the "true" leader at all times.
...
LoseAllLinks(n) ==
    /\ \A m \in N : LinkOk(n, m)
    /\ link' = [nd \in N |->
                    IF nd = n THEN [m \in N |-> IF nd = m THEN Up ELSE Down]
...
UncommittedCommitOnNode(n, t) ==
    /\ transaction[t].status = PendingCommitResult 
    /\ transaction[t].commit_node = n
    /\ transaction[t].commit_index > machine[n].committed_index

TransactionsDieOnNode(n) ==
    transaction' = [t \in T |->
                        IF transaction[t].node = n THEN
                            [transaction[t] EXCEPT !.status = Aborted]
                        ELSE IF UncommittedCommitOnNode(n, t)
                             THEN [transaction[t] EXCEPT !.status = PendingAbort]
                             ELSE transaction[t]]
...
MachineStarts(n) ==
    /\ machine' = [machine EXCEPT ![n] = [@ EXCEPT !.status = Follower]]
    /\ node_change_ctr' = node_change_ctr + 1
...
CanBeLeader(n, reachable_nodes) ==
    /\ SeesMajority(reachable_nodes)
    /\ \A node \in reachable_nodes : machine[n].last_index >= machine[node].last_index
        
\* Vastly simplied model where leader election is atomic and models only required aspects of Raft
\* Followers and partitioned nodes truncate their log to their committed index in order to 
\* simplify some other aspects of the spec (trying to avoid modelling Raft as much as possible)
\* A leader cannot be reelected in this model. In reality the current leader can win again, but we don't model that as there are no state changes.
MachineWinsLeaderElection(n, reachable_nodes) ==
    ...
    /\ machine' = [nd \in N |->
                    IF nd = n THEN
                        [machine[nd] EXCEPT !.status = Leader]
                    ELSE
                        [machine[nd] EXCEPT !.status     = Follower,
                                            !.log        = { e \in machine[nd].log : e.index <= machine[nd].committed_index },
                                            !.last_index = machine[nd].committed_index]]
    /\ leader' = n
    
StartAndStopLockers(n, l) ==
    /\ locker[l].status = NotStarted
    /\ locker' = [lk \in L |-> IF lk = l THEN [locker[lk] EXCEPT !.status = PendingNotifyUp,
                                                                 !.node = n,
                                                                 !.term = locker_next_term]
                                \* if the locker is monitored by the leader in the current state, then this machine tells the locker to stop
                                ELSE IF leader # 0 /\ locker[l].node = leader /\ machine[leader].monitored_locker = lk
                                     THEN [locker[lk] EXCEPT !.status = Stopped] 
                                     ELSE locker[lk]]
    /\ locker_next_term' = locker_next_term + 1

NotifyUncommittedTransactions(n) ==
    transaction' = [t \in T |->
                        IF leader # 0 /\ UncommittedCommitOnNode(leader, t)
                        THEN [transaction[t] EXCEPT !.status = PendingAbort]
                        ELSE transaction[t]]
...
PerformLeaderElection ==
    \E n \in N :
        ...
        IN /\ machine[n].status # Offline
           /\ MachineWinsLeaderElection(n, reachable_nodes)
           /\ \E l \in L : StartAndStopLockers(n, l)
           /\ NotifyUncommittedTransactions(n)
           /\ UNCHANGED << node_change_ctr, link, link_change_ctr, ooo_writes, write_log >>
...
ProcessCommitsTransaction(t) ==
    ...
    /\ transaction' = [transaction EXCEPT ![t] = [@ EXCEPT !.status = PendingCommitResult,
                                                           !.commit_node = leader,
                                                           !.commit_index = machine[leader].last_index + 1]]
...
ProcessAbortsTransactionCommit(t) ==
    /\ leader # 0
    /\ transaction[t].status = PendingAbort
    /\ LET l == transaction[t].locker
       IN
            \/ /\ ~TransactionLockerOk(l, t)
               /\ ProcessAbortsTransaction(t)
...
\* Last modified Tue Jan 07 14:34:45 CET 2020 by GUNMETAL
